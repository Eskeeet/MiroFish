...
class OasisProfileGenerator:
    ...
    def __init__(
        ...
        zep_api_key: Optional[str] = None,  # kept for API compatibility, unused
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        try:
            if self._local_tools is None:
                ...
                self._local_tools = LocalToolsService()

            query = f"关于{entity.name}的所有信息、活动、事件、关系和背景"
            sr = self._local_tools.search_graph(
            ...
            node_sr = self._local_tools.search_graph(
                graph_id=self.graph_id,
                query=query,
                limit=20,
                scope="nodes",
            )
            summaries = [n.get("summary", "") for n in node_sr.nodes if n.get("summary")]
            ...
            logger.info(f"本地检索完成: {entity.name}, 获取 {len(results['facts'])} 条事实")

        except Exception as e:
            logger.warning(f"本地检索失败 ({entity.name}): {e}")
